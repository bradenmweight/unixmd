...
class DFT_G09(Gaussian09):
    ...
    def __init__(self, molecule, nthreads=1, memory="1gb", functional="BLYP", basis_set="STO-3G", \
        guess="Harris", guess_file="./g09.chk", root_path="./", version="Revision A.02", dielectric_material=None):
        # Initialize Gaussian09 common variables
        super(DFT_G09, self).__init__(basis_set, memory, nthreads, root_path, version)
    ...
    def copy_files(self, molecule, istep, calc_force_only):
        ...
        if (molecule.nst > 1 and not calc_force_only and istep >= 0):
            if (istep == 0):
                ...
                shutil.copy( f"{self.scr_qm_dir}/g09.rwf", f"{self.scr_qm_dir}/../g09.rwf.pre" )
        ...
        if (self.guess == "read" and istep >= 0):
            # After T = 0.0 s
            shutil.copy( f"{self.scr_qm_dir}/g09.chk", f"{self.scr_qm_dir}/../g09.chk.pre" )
    ...
    def get_input(self, molecule, istep, bo_list, calc_force_only):
        ...
        if (self.guess == "read"):
            if (istep == -1):
                if (os.path.isfile(self.guess_file)):
                    ...
                    shutil.copy( self.guess_file, f"{self.scr_qm_dir}/g09.chk" )  # os.path.join(self.scr_qm_dir, "g09.chk"))
                    restart = True
                else:
        ...
        if( basis_header == 'GEN' ):
            input_g09 += basis_list + "\n"
        else:
            input_g09 += "\n"
        if( ECP_out != '' ): input_g09 += ECP_list + "\n"
        if ( solvent_method != '' ): input_g09 += solvent_list + "\n"
        input_g09 += "\n\n\n"
...
%kjob l302
%rwf=g09_double.rwf
# {self.functional}/{basis_header} {ECP_out} \n# {solvent_method} IOp(2/12=3) nosymm\n\n""")
    ...
    def run_QM(self, base_dir, istep, bo_list):
        ...
        if (istep == -1):
            ...
            path_profile =  f"{self.root_path}/bsd/g09.profile"  #os.path.join(self.root_path, "/bsd/g09.profile") # BMW, OLD = "g09/bsd/g09.profile"
            ...
            """
        # Set run command
        qm_command = f"{self.root_path}/g09"    # os.path.join(self.root_path, "g09/g09") # BMW
    ...
    def CI_overlap(self, molecule, istep, dt):
        """ Read the necessary files and calculate NACME from tdnac.c routine
            ...
            :param double dt: Time interval
        """
        path_rwfdump = f"{self.root_path}/rwfdump" # os.path.join(self.root_path, "/rwfdump") # BMW, OLD = "g09/rwfdump"
