...
class DFT_SPLITBASIS(QChem):
    ...
    def __init__(self, molecule, basis_set="sto-3g", memory=2000, nthreads=1, \
        ...
        cpscf_max_iter=30, cpscf_grad_tol=6, root_path="./", version="5.2", dielectric_const=None):
        # Initialize Q-Chem common variables
        super(DFT_SPLITBASIS, self).__init__(basis_set, memory, root_path, nthreads, version)
    ...
    def get_input(self, molecule, bo_list, calc_force_only):
        ...
        def getSplitBasis(basis_set):
            ...
            ECP_list = ['LANL2DZ', 'DEF2-ECP'] # Manually add here to search for more. Otherwise, an error will occur.
            ECP_out = ''
            ...
            return basis_set, basis_list, ECP_out
        ...
        if ( "//" in self.basis_set ):
            print ("Found split basis set")
            QSplitBasis = True
            basis_set, basis_list, ECP_out = getSplitBasis(self.basis_set)
            self.basis_set = basis_set

        # Indicates that user wants a solvent cavity. Only CPCM is implemented with
        #   choice of dielectric constant.
        solvent_method = ""
        if ( self.dielectric_const != None ):
            solvent_method = "SOLVENT_METHOD  PCM"
            solvent_list = textwrap.dedent(f"""\
            $pcm
               Theory          CPCM
               Method          SWIG
               Solver          Inversion
            ...
            $solvent
               Dielectric {self.dielectric_const}
            $end
            """)
        ...
        if (not calc_force_only and self.calc_coupling):
            ...
            { ECP_out }
            { solvent_method * (self.dielectric_const != None) }
            ...
            {basis_list * (self.basis_set == 'General') }

            {solvent_list * (self.dielectric_const != None) }
    ...
    def run_QM(self, base_dir, istep, bo_list):
        ...
        path_qcenv = f"{self.root_path}/qcenv.sh"  # os.path.join(self.root_path, "qcenv.sh") # BMW
        
        # I COMMENTED THIS PART OUT. SEEMS TO WORK FINE WITHOUT IT... BMW
